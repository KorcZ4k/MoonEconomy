...
# Collection já existente. Não cria uma nova collection com outro nome.
mora = db["Mora"]
...
def pagar_mora(remetente_id: int, destinatario_id: int, guild_id: int, quantidade: int):
    ...
    resultado = mora.update_one(
        {"ID": str(remetente_id), "guild_id": str(guild_id), "carteira": {"$gte": quantidade}},
        {"$inc": {"carteira": -quantidade}},
    )
    if resultado.modified_count == 0:
        raise ValueError("Mora insuficiente.")
    mora.update_one(
        {"ID": str(destinatario_id), "guild_id": str(guild_id)},
        {"$inc": {"carteira": quantidade}},
        upsert=True,
    )
    return True
...
def ranking_mora(guild_id: int, limite: int = 10):
    return list(mora.find({"guild_id": str(guild_id)}).sort([("carteira", -1), ("banco", -1)]).limit(limite))


def economia_mora(guild_id: int):
    resultado = list(mora.aggregate([
        {"$match": {"guild_id": str(guild_id)}},
        {"$group": {"_id": None, "carteira_total": {"$sum": "$carteira"}, "banco_total": {"$sum": "$banco"}, "jogadores": {"$sum": 1}}},
    ]))
    if not resultado:
    ...
    return {"carteira_total": carteira_total, "banco_total": banco_total, "total": carteira_total + banco_total, "jogadores": dados.get("jogadores", 0)}
